...
D <- as.numeric( (runif(bigN, 0, 1)) > 0.5) # if above 0.5 in treatment group, below 0.5 in control group
mean(D) 
...
# "TEST" OF COVARIATE BALANCE (confirm that randomization worked)
sumtable(RCT_DATA, 
         vars = c('Y0','Y1', 'Y', 'X1', 'X2', 'X3', 'X4', 'X5'), 
         group = 'D', group.test = TRUE)
# cannot reject null hypothesis based on the summary statistics F test (look at ***s)
# "0 mean" and "1 mean" are similar - means balancing property 
# can't observe Y0 and Y1 in the real world
# can reject the null that the difference is equal to 

# another way to confirm that randomization worked
mA <- lm(formula = X ~ D, data = RCT_DATA) # regress X (# from 1 to 5) on D
...
se_models = starprep(ate1, ate2, 
                     stat = c("std.error"), 
                     se_type = "HC2", 
                     alpha = 0.05)
stargazer(ate1, ate2, se = se_models, type = "text")
# standard error decreases in 2nd model (regressing D and X on Y)
